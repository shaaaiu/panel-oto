...
// =====================================================
// PTERODACTYL FUNCTIONS (SAMA PERSIS SEPERTI REQUIME VERSION)
// =====================================================
async function pteroCreateOrGetUser({ email, username, password }) {
    ...
    if (createRes.ok && !createJson.errors) {
        return createJson.attributes;
    }
    ...
    if (!user) {
        throw new Error("Pterodactyl user error");
    }
...
async function pteroGetEggStartup() {
    const res = await fetch(`${CONFIG.PTERO_DOMAIN}/api/application/nests/${CONFIG.NEST_ID}/eggs/${CONFIG.EGG_ID}`, {
        headers: {
            'Accept': 'application/json',
            'Authorization': `Bearer ${CONFIG.PTERO_APP_KEY}`
        }
...
async function pteroCreateServer({ userId, name, memo, cpu, eggId, startup, locId }) {
    ...
    if (!res.ok || json.errors) {
        throw new Error(json?.errors?.[0]?.detail || "Gagal membuat server");
    }
...
app.post('/api/order', async (req, res) => {
    try {
        ...
        if (!chosen) {
            return res.json({ ok: false, error: "Paket tidak ditemukan" });
        }
        ...
        const qrPng = await QRCode.toDataURL(pay.qr);
